...
public class ShowResults extends JPanel implements ActionListener {
    ...
    public ShowResults(SharkPanel _sp, String _files[], double _scores[], int _nrpairs[]) {
    	...
    	System.out.println("ShowResults");//Daniel Remove
    	
        sp = _sp;
		
		log = I3SMPrefs.getLogging();
		...
		if(log)
			lf = new LogFile("SearchResults_");
        DefaultListModel listModel = new DefaultListModel();
        for(int i=0; i<files.length; i++) {
			if(scores[i] >= 1000000.0)
				continue;
			nrResults++;
        ...
        if(frame != null)
            frame.dispose();
        frame = new JFrame("I3S Manta: Search results");
