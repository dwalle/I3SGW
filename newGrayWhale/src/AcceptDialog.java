...
class AcceptDialog extends JPanel implements ActionListener {
    ...
    public AcceptDialog(CompareResults _cr, String _forig, String _ffound) {
        ...
        if(index == -1) {
            JOptionPane.showMessageDialog(null, "Internal error: copy image and fingerprint file \nmanually to proper directory in the database");
            frame.dispose();
            return;
        }
        ...
        label = new JLabel(" Database folder:  " + reduceString(dir));
        label.setFont(new Font("Arial", Font.PLAIN, 11));
        wrapper.add(label, BorderLayout.LINE_START);
...
*/
		// Renaming sub pane
        wrapper = new JPanel();
        wrapper.setBorder(BorderFactory.createTitledBorder("Naming of the individual"));
        wrapper.setLayout(new GridLayout(0, 1));
        ...
        frame = new JFrame("I3S Gray Whale: Include in database");
